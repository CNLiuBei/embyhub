...
const UserLayout = () => {
  ...
  const userRoleName = roleNames[user?.role || 0] || '普通用户'

  const menuItems = [
    { key: '/user', icon: <UserOutlined />, label: '个人中心' },
    { key: '/user/member', icon: <CrownOutlined />, label: '会员中心' },
    { key: '/user/media', icon: <VideoCameraOutlined />, label: '媒体库' },
    { key: '/user/points', icon: <TrophyOutlined />, label: '积分中心' },
    { key: '/user/forum', icon: <CommentOutlined />, label: '社区论坛' },
    { key: '/user/messages', icon: <MessageOutlined />, label: '消息中心' },
    { key: '/user/invite', icon: <GiftOutlined />, label: '邀请好友' },
    { key: '/user/notifications', icon: <BellOutlined />, label: '系统通知' },
  ]
  ...
  return (
    <Layout style={{ minHeight: '100vh', background: 'var(--bg-base, #f5f7fa)' }}>
      {/* 卡片式侧边栏 */}
      <div className={`fixed left-4 top-4 bottom-4 w-[220px] rounded-2xl shadow-lg overflow-hidden flex flex-col max-md:hidden ${glassBg} backdrop-blur-xl border ${glassBorder}`}>
        {/* Logo */}
          ...
          <div className="flex items-center gap-3 min-w-0">
            {siteSettings.logo ? (
              <img
                ...
                alt="Logo"
                className="w-9 h-9 rounded-full object-contain shadow-lg flex-shrink-0"
              />
            ) : (
            ...
            <div className={`flex items-center gap-3 p-2 rounded-xl cursor-pointer ${hoverBg} transition-all duration-200`}>
              ...
              {!collapsed && (
                <div className="flex-1 min-w-0">
                  <div className={`${textColor} font-semibold text-sm truncate`}>{user?.nickname || user?.username}</div>
                  <div className={`${textColorMuted} text-xs`}>{userRoleName}</div>
                </div>
              )}
            </div>
          </Dropdown>
      ...
      {/* 内容区域 - 固定卡片，内容滚动 */}
      <div className={`fixed left-[252px] top-4 bottom-4 right-4 rounded-2xl shadow-lg overflow-hidden flex flex-col max-md:hidden ${glassBg} backdrop-blur-xl border ${glassBorder}`}>
        ...
        {menuItems.slice(0, 4).map((item) => (
          <div
            ...
            className={`flex-1 flex flex-col items-center justify-center cursor-pointer transition-colors ${
              location.pathname === item.key ? 'text-blue-500' : `${textColorMuted} hover:${textColor}`
            }`}
          >
